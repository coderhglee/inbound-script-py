...
class SftpClient(object):
    ...
    def file_list_helper(self, sftp, files):
        ...
        for attr in stats:
            ...
            if S_ISDIR(attr.st_mode):  # If the file is a directory, recurse it
                ...
                self.file_list_helper(sftp, files)
                sftp.chdir('..')
    ...
    def get(self, remote, local, filename):
        ...
        if exist_local_file("%s/%s" % (local, filename)):
    ...
    def get_all(self, remote_list):
        ...
        for folder, files in remote_list:
            ...
            self.logger.info("Get All Files, remote Directory files size: %s" % str(len(files)))
            # 폴더구조를 똑같이 복사하기 위한.
            # 루트 디렉토리 문자열 제거.
            ...
            if create_local_folder(folder=target_folder):
                ...
                for file in files:
                    ...
                    # 로컬에 이미 파일이 존재한다면.
                    if exist_local_file(local_file):
                        # 최종 수정시간 비교후 저장.
                        remote_file_time = ms_turc(self.sftp.stat(remote_file).st_mtime)
                        local_file_time = ms_turc(get_stat_file(local_file).st_mtime)
    ...
    def del_file(self, file):
        try:
            ...
            self.logger.info('SUCCESS Remove sftp file: {%s}' % file)
        except Exception as e:
